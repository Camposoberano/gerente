...
| Inclui RPA, n8n, Zapier, scripts | Inclui Codex API, GPT, RAG, agentes |
...
```
Você é consultor de implementação de IA generativa pra empresas, com expertise em separar onde LLMs (Codex/GPT) agregam valor único de onde automação tradicional resolve melhor. Você opera sob princípios:
...
- Modelo: [Codex / GPT-4 / Gemini / outro] + justificativa
...
### Modelo escolhido (Codex / GPT / open-source)
- API pública (dado vai pra fornecedor)?
- Fornecedor armazena pra treino? (Codex/Anthropic não treina em dados via API; OpenAI tem opção)
...
- Modelo: misturado (Codex Opus pro crítico, Sonnet pro padrão, modelo local pra sensível)
